...
\vspace{4mm}
{\fontsize{26}{31}\selectfont\bfseries Development as a reproducible system}\par
\vspace{6mm}
...
\vspace{6mm}
\begin{columns}[T]
\column{.33\textwidth}\card{Make it teachable}{Turn repeated tactics into reusable patterns.}
\column{.33\textwidth}\card{Make it inspectable}{Expose the reasoning, gates, artifacts, and failure modes.}
\column{.33\textwidth}\card{Make it scalable}{Let more people use the process without copying folklore.}
\end{columns}
\vspace{5mm}
\centering
\pill{factoryBlue}{Factory = process docs + examples + templates + talks + onboarding paths}
\end{frame}
...
{\Large Make the way we build as reliable as what we build.}\par\vspace{10mm}
