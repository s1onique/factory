...
\begin{frame}{Factory is not a framework}
\large

Factory is a way to build, run, and evolve feedback loops around software delivery.

\vspace{5mm}

It does not prescribe your stack, tools, process, architecture, team shape, or rituals.

\vspace{4mm}

\begin{columns}[T]
\column{.33\textwidth}
\card{Change everything}{Tools, agents, tests, repos, workflows, prompts, and gates are replaceable.}

\column{.33\textwidth}
\card{Keep the loops}{The durable part is the habit of creating, maintaining, and inspecting feedback loops.}

\column{.33\textwidth}
\card{Execute the process}{Factory is not just documentation. It is a living system that runs, learns, and improves.}
\end{columns}

\vspace{5mm}

\centering
\pill{factoryBlue}{Factory = feedback-loop meta-builder + executor}

\end{frame}

\begin{frame}{Factory chooses the Bazaar}
\large

Factory is not a cathedral architecture.

\vspace{4mm}

It does not ask us to design the perfect process and live inside it.

\vspace{5mm}

Factory creates conditions where many small feedback loops can appear, compete, mutate, survive, or die.

\vspace{5mm}

\begin{columns}[T]
\column{.33\textwidth}
\card{Variation}{Many loops, tools, prompts, gates, examples, and tactics can coexist.}

\column{.33\textwidth}
\card{Selection}{Tests, users, incidents, reviews, traces, and deployments decide what is useful.}

\column{.33\textwidth}
\card{Memory}{The winners become scripts, docs, patterns, fixtures, talks, and defaults.}
\end{columns}

\vspace{5mm}

\centering
\pill{factoryBlue}{Factory makes software work evolvable}
