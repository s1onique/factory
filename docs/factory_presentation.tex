...
Before AI, many valuable engineering tasks were technically possible but economically irrational:
writing thousands of tests, exhaustively hardening edge cases, replaying failures, documenting every pattern,
and reviewing every boring change with full attention.

\vspace{5mm}

\begin{columns}[T]
\column{.33\textwidth}
\card{Compress the loop}{Move from idea to patch to verification in minutes, not days.}

\column{.33\textwidth}
\card{Keep control}{Use tests, specs, gates, traces, and artifacts to constrain AI-generated work.}

\column{.33\textwidth}
\card{Scale the method}{Capture the process so more people can operate the same factory.}
\end{columns}

\vspace{5mm}
\centering
\pill{factoryBlue}{Factory = a way to turn AI speed into reliable software delivery}
\end{frame}
