...
// MICROFIX: WitnessSpawnHandle moved to witness-start-handle.ts
// (source-discipline closure). Import it for internal-only use
// (StartedWitness.child, WitnessSpawnSpecResult.handle); the
// public re-export at the bottom of this file preserves the
// existing import surface for callers.
import type { WitnessSpawnHandle } from "./witness-start-handle.js";
...
/**
 * Re-exports for the post-spawn handle types so callers
 * importing from witness-start-types keep working. The
 * implementation lives in witness-start-handle.ts
 * (extracted in MICROFIX source-discipline closure to keep
 * this file under the 400-LOC discipline).
 *
 *   WitnessBootstrapOutput  — bounded stdio evidence
 *   WitnessExitInfo         — final child completion
 *   WitnessSpawnHandle      — narrow handle abstraction
 *
 * This re-export is a strictly behavioral no-op: the
 * symbols here resolve to the SAME runtime types they
 * resolved to before the extraction. Tests that import
 * directly from witness-start-types continue to compile
 * and to assert the same shapes.
 */
export type {
